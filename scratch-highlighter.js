/*
 * Language: Scratch
 * Description: Syntax highlighting for Scratch blocks written as text
 * Usage: hljs.registerLanguage('scratch', scratchHighlighter);
 */

function scratchHighlighter(hljs) {
  const SINGLE_WORD_KEYWORDS = [
    'repeat', 'forever', 'if', 'else', 'wait', 'until', 'while',
    'stop', 'end', 'then', 'times', 'seconds', 'when', 'broadcast'
  ];

  const SINGLE_WORD_BUILT_INS = [
    'move', 'turn', 'glide', 'say', 'think', 'show', 'hide',
    'change', 'set', 'add', 'delete', 'insert', 'item',
    'myself', 'touching', 'ask', 'answer', 'timer', 'loudness'
  ];
  ...
  return {
    ...
    contains: [
      ...
      },
      {
        ...
        contains: ['self', hljs.COMMENT('//', '$')]
      }
    ]
  };
}

// For use in CodePen or direct browser inclusion:
